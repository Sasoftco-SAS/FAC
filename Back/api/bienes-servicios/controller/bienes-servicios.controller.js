...
const BienesDao = require('../dao/bienes-servicios.dao');
 ...
 */
 function create(req, res, next) {
    try {
        ...
        let objeto_bienes = req.body;
        let objObj = JSON.parse(JSON.stringify(objeto_bienes));

        //console.log(objObj); //BORRAR

        BienesDao['create'](objObj).then(async _obj => {
            res.status(201).json({ "bien_servicio": _obj });
        });
    } catch (err) {
