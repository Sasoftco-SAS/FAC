 ...
 * @param next
 */
const create = async (req, res, next) => { //Se convierte en un ArrayFunction con Const al inicio para poder usar Async await
  try {
    ...
    let userId = req.user.sub.user._id;
    const unidad = await UnidadDao.findOne({ rResponsable: userId }).exec(); //Hasta que no se resuelva esta promesa no continua a buscar la necesidad
    const necesidades = await NecesidadDao.find({ unidad: unidad._id }).exec(); //Esto gracias a Async await 
    const lastNecesidad = necesidades.reverse()[0];
    const codeUnidad = unidad.name.substr(0, 3).toUpperCase() + "_";
    if (lastNecesidad?.consecutivo) {
      const lastConse = lastNecesidad.consecutivo.replace(codeUnidad, "");
      objObj.consecutivo = `${codeUnidad}${parseInt(lastConse) + 1}`;
    } else {
      objObj.consecutivo = `${codeUnidad}1`;
    }
    objObj.unidad = unidad;
    NecesidadDao["create"](objObj).then(async (_obj) => {
      res.status(201).json({ necesidad: _obj });
    });
  } catch (err) { //Si no resuelve alguna de las dos promesas anteriores entra acá
    console.log(err);
...
//--------
/**
 * Método para obtener necesidad por id NO FUE HECHO.
 * @param req
 * @param res
 * @param next
 */
